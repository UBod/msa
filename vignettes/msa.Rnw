...
\author{Enrico Bonatesta\affilmark{2}, Christoph Kainrath\affilmark{2}, and Ulrich Bodenhofer\affilmark{1,2}}
...
\manualtitlepage{Version \msaVer, \msaDate}{https://github.com/UBod/msa}
...
although the \verb+MultipleAlignment+ package is providing data types for
representing multiple sequence alignments (up to Bioconductor version 3.23, these were provided by the
\verb+Biostrings+ package).
...
\subsection{Methods Inherited From {\tt MultipleAlignment}}
...
\verb+MultipleAlignment+ package (up to Bioconductor version 3.23, these were
provided by the \verb+Biostrings+ package). Therefore, all methods for processing
...
The \verb+unmasked()+ function allows for removing these masks, thereby casting
...
the \verb+MultipleAlignment+ package. However, for more flexibility and consistency,
...
\verb+consensusString()+ method from the \verb+MultipleAlignment+ package:
<<consensusExample3>>=
...
The packages \verb+Biostrings+ and \verb+MultipleAlignment+ do not impose
any restrictions on the names of
...
On some systems (e.g.\ FreeBSD), the required Gnu make command is installed under a
different name, e.g., \verb+gmake+. The \MSA\ package installation can handle this under
the condition that an environment variable \verb+MAKE+ is defined beforehand that contains
the name of the Gnu make binary.
